...
public class DataSiswaDuplicate {
     ...
     */
    private static final CreateDataSiswaDup crData = new CreateDataSiswaDup();
    /**
     * File yang berisi data Siswa.
     */
    public static final String filename = "D:\\App Data\\SPP Payment\\data\\Data Siswa.haqi";
    /**
     * Direktori dari penyimpanan foto profile dari siswa.
     */
    public static final String picDir = "D:\\App Data\\SPP Payment\\pictures\\";
     ...
     * yang berada diclass <code>ValidationSiswaDup</code>. Jika output dari method tersebut adalah <strong>True</strong>
        ...
        try{
            ...
            while((buffer = baca.readLine()) != null){
                ...
                if(buffer.length() >= 1 || !buffer.startsWith("//")){
                    ...
                    if(ValidationSiswaDup.isValidData(buffer)){
                        showData(buffer);
                    }else{
     ...
     * yang berada didalam class <code>ValidationSiswaDup</code> jika output dari method tersebut adalah <strong>False</strong>
    ...
    public static void showData(String data){
        ...
        if(ValidationSiswaDup.isValidData(data)){
            // mendapatkan data
            ...
            System.out.println(
                    String.format(
                            "NIS : %s\nNama : %s\nGender : %s\nTTL : %s\nAlamat : %s\nKelas : %s\nNomor Telepon : %s\nEmail : %s\nNama Wali : %s\nID SPP : %s\nFoto Profile : %s\n", 
                            nis, crData.toCapitalize(nama), crData.getGenderName(gender), tmpLahir + ", " + waktu.getTanggal(tglLahir), alamat, kls.getNamaKelas(kelas), nohp, gmail, ortu, spp, foto
                    )
            );            
        }else{
     ...
     * <code>isValidData(String data)</code> yang berada didalam class <code>ValidationSiswaDup</code>. 
     * 
     * <br><br>
     ...
     */
    public static void chekingData(final boolean showValid, final boolean showInValid){
        try{
     ...
     * dengan menggunakan method <code>isValidData(String data)</code> yang berada diclass <code>ValidationSiswaDup</code>.
     ...
     * 
     */
    public static void repairAllData(){
        try{
     ...
     * method <code>isValidData(String data)</code> yang berada diclass <code>ValidationSiswaDup</code> 
     * <br><br>
     ...
     */
    public static String repairData(String data){
        ...
        if(data.length() >= 1 && !data.startsWith("//")){
            ...
            if(!ValidationSiswaDup.isValidData(data)){
                ...
                if(token.countTokens() < 4 || !ValidationSiswaDup.isValidPrimaryData(data)){
                    return null;
                }else{
                    // memperbaiki data 
        ...
        // mengecek data
        if(ValidationSiswaDup.isValidData(newData)){
    ...
    private static String repairData0(String data){
        ...
        // mencari data mana yang tidak valid
            ...
            if(!ValidationSiswaDup.isNamaTempat(tmpLahir)){
                tmpLahir = crData.randTempat();
            }
            // mengecek apakah tanggal lahir valid atau tidak
            if(!ValidationSiswaDup.isTanggalLahir(tglLahir)){
                tglLahir = crData.randTanggalLahir();
            }
            // mengecek apakah alamat valid atau tidak
            if(!ValidationSiswaDup.isNamaTempat(alamat)){
                alamat = crData.randTempat() + ", Indonesia";
            }
            // mengecek apakah nomor hp valid atau tidak
            if(!ValidationSiswaDup.isNoHp(nohp)){
                nohp = crData.randNoHp();
            }
            // mengecek apakah gmail valid atau tidak
            if(!ValidationSiswaDup.isEmail(gmail)){
                gmail = crData.createGmail(nama);
            }
            // mengecek apakah ortu valid atau tidak
            if(!ValidationSiswaDup.isNamaOrtu(ortu)){
                ortu = crData.randNamaOrtu();
            }
            // mengecek apakah spp valid atau tidak
            if(!ValidationSiswaDup.isIDSpp(spp)){
                spp = crData.getIDSpp(kelas);
            }
            // mengecek apakah foto profile valid atau tidak
            if(!ValidationSiswaDup.isProfile(foto)){
                foto = crData.getProfile(nis, nama);
            }
            // mengembalikan data baru yang sudah direpair
            return String.format(
    ...
    /**
     * Method ini digunakan untuk mengosongkan semua data yang ada didalam tabel siswa dan users.
     ...
     */
    public static void setEmptyData(){
        try{
            // menghapus semua data yang ada didalam tabel siswa dan users
            siswa.stat.executeUpdate("TRUNCATE siswa;");
            siswa.stat.executeUpdate("DELETE FROM users WHERE level = 'SISWA'");
        }catch(SQLException ex){
            System.out.println("Terjadi Kesalahan : " + ex.getMessage());
        }
    }
    ...
    /**
     * Method ini digunakan untuk mengecek apakah tabel siswa dan user kosong atau tidak.
    ...
    public static boolean isEmptyData(){
        try{
            int cSiswa = -1, cUsers = -1;
            String tSiswa = "SELECT COUNT(nis) AS total FROM siswa;",
                   tUsers = "SELECT COUNT(id_user) AS total FROM users WHERE level = 'SISWA';";
            
            // mendapatkan jumlah data yang ada didalam tabel siswa
            siswa.res = siswa.stat.executeQuery(tSiswa);
            if(siswa.res.next()){
                cSiswa = siswa.res.getInt("total");
            }
            
            // mendapatkan jumlah data yang ada didalam tabel users
            siswa.res = siswa.stat.executeQuery(tUsers);
            if(siswa.res.next()){
                cUsers = siswa.res.getInt("total");
            }
            
            return cSiswa == 0 && cUsers == 0;
        }catch(SQLException ex){
            System.out.println("Terjadi Kesalahan : " + ex.getMessage());
        }
        return false;
     ...
     * <code>isValidAllData()</code> yang berada didalam class <code>ValidationSiswaDup</code>.
     * <br><br>
                ...
                if(ValidationSiswaDup.isValidAllData()){
                    ...
                    while((buffer = reader.readLine()) != null){
                        ...
                        if(buffer.length() >= 1 && !buffer.startsWith("//")){
                            ...
                            if(isSuccess){
                                success++;
                                System.out.println("Data ke-"+ data +" pada baris " + baris + " berhasil ditransfer.");
                            }else{
                                failed++;
                                System.out.println("Data ke-"+ data +" pada baris " + baris + " gagal ditransfer.");
                            }
                            finish = System.nanoTime();
                            System.out.format("Transfer Time : %.2f detik \n\n", (double)(finish-start)/1_000_000_000L);
                            data++;
                        }else{
                            Log.addLog("Data pada baris " + baris + " diskip karena merupakan data kosong!");
                            System.out.println();
                        }
                        baris++;
                    }
                    ...
                    // memperbaiki data yang tidak valid
                    DataSiswaDuplicate.repairAllData();
                    // mencoba mentransfer ulang data
                    DataSiswaDuplicate.transferAllData();
                }
            }else{
                Log.addLog("Harap kosongkan tabel siswa secara manual!");
            }
     ...
     * diclass <code>ValidationSiswaDup</code>.
     * <br><br>
     ...
     */
    public static boolean transferData(String data){
        try{
            // object dan variabel yang digunakan 
            File fotoProfile;
            StringTokenizer token = new StringTokenizer(data, "#");
            ...
            if(ValidationSiswaDup.isValidData(data)){
                ...
                if(foto.equalsIgnoreCase("default.png")){
                    fotoProfile = null;
                }else{
                    fotoProfile = new File(picDir+foto);
                }
                
                return siswa.addSiswa(nis, nohp, gmail, fotoProfile, nama, gender, tmpLahir, tglLahir, alamat, kelas, ortu, Integer.parseInt(spp));
            }else{
                Log.addLog("Gagal mentransfer data dikarenakan data tidak valid!");
            }
            
        }catch(NumberFormatException | FileNotFoundException | SQLException | InValidUserDataException ex){
            System.out.println("Terjadi Kesalahan : " + ex.getMessage());
        }
        return false;
    }
    ...
    /**
     * Method ini digunakan untuk membuat sebuah bot Siswa yang akan digunakan untuk menguji coba 
     * aplikasi apakah nantinya akan berjalan dengan lancar atau tidak. Data-data pada bot Siswa 
     * akan diisi secara random. Sebelum membuat bot method akan mengecek apakah id kelas yang 
     * diinputkan valid atau tidak dengan menggunakan method <code>isKelas(String kelas)</code> yang 
     * berada didalam class <code>ValidationSiswaDup</code>.
     * <br><br>
     * Jika output dari method tersebut adalah <strong>False</strong> maka bot tidak akan dibuat.
     * Tapi jika outputnya <strong>True</strong> maka bot akan dibuat dengan cara merandom data-data 
     * dari siswa melalui class <code>CreateDataSiswaDup</code>. Jika data-data dari siswa berhasil dibuat 
     * maka selanjutnya mentransfer data tersebut kedalam <code>Database</code> melalui method 
     * <code>transferData(String data)</code>.
     * 
     * @param idKelas ID Kelas dari bot Siswa.
     * @param jumlah Jumlah bot akan dibuat.
     */
    public static void createBot(String idKelas, int jumlah){
        ...
        if(ValidationSiswaDup.isKelas(idKelas)){
            for(int i = 1; i <= jumlah; i++){
                ...
                DataSiswaDuplicate.transferData(data);
            }
            
        }else{
            System.out.println("Gagal membuat bot karena ID Kelas tidak valid!");
        }
    }
    
    /**
     * Method ini digunakan untuk mentransfer data-data foto profile ke dalam database.
     * Petama-tama method akan membuat sebuah object <code>PreparedStatement</code> yang 
     * digunakan untuk mengirimkan data foto profile dari user ke <code>Database</code>.
     * Selanjutnya method akan mengecek apakah ID/NIS dan foto profile Siswa valid 
     * atau tidak dengan menggunakan method <code>isNis(String nis)</code> dan 
     * <code>isProfile(String foto)</code> yang berada didalam class <code>ValidationSiswaDup</code>.
     * <br><br>
     * Jika ID/NIS dan foto profile valid maka method akan merubah file foto yang dinputkan 
     * menjadi tipe data blob yang digunakan untuk meyimpan sebuah file kedalam <code>SQL</code>.
     * Tapi jika ID/NIS dan foto profile tidak valid maka foto profile akan diset nilai NULL. 
     * 
     * 
     * @param id NIS dari Siswa.
     * @param foto Direktori dari foto profile Siswa.
     * @return <strong>True</strong> jika foto profile berhasil ditransfer. <br>
     *         <strong>False</strong> jika foto profile gagal ditransfer.
     */
    public static boolean transferFotoProfile(String id, String foto){
        try {
            // membuat object PreparedStatement
            PreparedStatement pst = siswa.conn.prepareCall("INSERT INTO foto_profile VALUES (?, ?)");
            // mengecek apakah id user sudah exist atau tidak
            if(!siswa.isExistUser(id)){
                // mengecek apakah nis dan foto profile valid atau tidak
                // jika foto memiliki nilai default.png maka data pada blob akan diset ke null
                if(ValidationSiswaDup.isNis(id) && ValidationSiswaDup.isProfile(foto) && !foto.equalsIgnoreCase("default.png")){
                    System.out.println(picDir+foto + " is exist");
                    // menginputkan nilai 
                    pst.setString(1, id);
                    pst.setBlob(2, new FileManager().fileToBlob(new File(picDir+foto))); // merubah file menjadi blob
                    // mengeksekusi query
                    return pst.executeUpdate() > 0;
                }else{
                    System.out.println(picDir+foto + " not exist");
                    // menginputkan nilai
                    pst.setString(1, id);
    ...
    public static void main(String[] args) {
        ...
        Log.createLog();
        DataSiswaDuplicate.showAllData();
        
    }
    
    private static void downloadAllPhotos(String destination) throws SQLException, IOException{
        
        Blob blob;
        String user, filenm;
        siswa.res = siswa.stat.executeQuery("SELECT * FROM foto_profile");
        
        while(siswa.res.next()){
            user = siswa.res.getString("id_user");
            blob = siswa.res.getBlob("foto");
            
            if(blob != null){
                filenm = destination + user + ".jpg";
                System.out.println("Mendownload file " + filenm);
                new FileManager().blobToFile(blob, filenm);
                System.out.println(filenm+" behasil didownload...");
            }
        }
    }
 ...
 * @since 2021-05-21
 */
class CreateDataSiswaDup extends DataSiswaDuplicate{
    ...
    /**
     * Digunakan untuk mendapatkan nama dari kode gender yang diinputkan kode gender
     * yang diinputkn harus diantara L atau P jika kode gender yang diinputkan bukan diantara
     * L atau P maka method akan mengembalikan nilai Unknown.
     * 
     * @param gender kode gender.
     * @return nama dari kode gender yang diinputkan.
     */
    protected String getGenderName(String gender){
        switch(gender){
     ...
     * profile Siswa.
     * <br><br>
     * Kemudian method akan mengecek apakah direktori tersebut exist atau tidak pada directory yang diinputkan. 
     * Jika direktori tersebut exist maka method akan mengembalikan nilai dari direktori tersebut. 
     * Tapi jika direktori tidak exist maka method akan mengembalikan nilai default dari foto profile yaitu NULL.
     * 
     * @param directory Direktori dari tempat penyimpanan foto.
    ...
    protected String getProfile(String nis, String nama){
        ...
        String directory = picDir + nis + ("_" + nama.replaceAll(" ", "_")) + ".jpg";
 ...
 * @since 2021-05-20
 */
class ValidationSiswaDup extends DataSiswaDuplicate{
    ...
    public static boolean isNamaSiswa(String nama){
        return nama.endsWith("\b") || nama.length() >= 5 && nama.length() <= 50 && !Validation.containsNumber(nama);
    }
    ...
    public static boolean isKelas(String kelas){
        System.out.println("Mengecek id kelas " + kelas + " exist atau tidak.");
    ...
    public static boolean isProfile(String foto){
        return new File(picDir+foto).exists();
    }
